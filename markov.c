//
// Created by amadeo on 21/10/2025.
//
// markov.c — TI301 PARTIE 1 : listes d'adjacence, vérification, export Mermaid
#include "markov.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static Cell* new_cell(int to, float p, Cell* next) {
    ...
    c->to = to; c->p = p; c->next = next;
    return c;
}

static void list_init(List* L) { L->head = NULL; }
static void list_push_front(List* L, int to, float p) { L->head = new_cell(to, p, L->head); }
...
void adj_print(const AdjList* G) {
    for (int i = 0; i < G->n; ++i) {
        ...
        printf(" [head @]");
        while (c) { printf(" -> (%d, %.2f)", c->to, c->p); c = c->next; }
        putchar('\n');
    }
}
...
AdjList readGraph(const char* filename) {
    ...
    if (!f) { perror("Could not open file for reading"); exit(EXIT_FAILURE); }
    ...
    while (fscanf(f, "%d %d %f", &depart, &arrivee, &proba) == 3) {
        if (depart < 1 || depart > n || arrivee < 1 || arrivee > n) {
            fprintf(stderr, "Avertissement: arc (%d -> %d) hors bornes [1..%d], ignoré.\n", depart, arrivee, n);
            continue;
        }
        if (!(proba >= 0.0f && proba <= 1.0f)) {
            fprintf(stderr, "Avertissement: probabilité %.6f hors [0,1], arc (%d -> %d) ignoré.\n", proba, depart, arrivee);
            continue;
        }
...
bool verify_markov(const AdjList* G, float lo, float hi) {
    ...
    for (int i = 0; i < G->n; ++i) {
        ...
        if (!(s >= lo && s <= hi)) {
            printf("Le graphe n’est pas un graphe de Markov : la somme des probabilités du sommet %d est %.6f\n", i+1, s);
            ok = false;
        }
    }
    if (ok) printf("✅ Le graphe est un graphe de Markov (tolérance [%.2f, %.2f]).\n", lo, hi);
    return ok;
}
...
void getId(int num, char* buf, size_t sz) {
    ...
    while (num > 0 && pos < (int)sizeof(tmp)-1) {
        num--; tmp[pos++] = (char)('A' + (num % 26)); num /= 26;
    }
    int j = 0;
...
bool write_mermaid(const AdjList* G, const char* outpath) {
    ...
    if (!out) { perror("open mermaid"); return false; }
    ...
    for (int i = 0; i < G->n; ++i) {
        char id[32]; getId(i+1, id, sizeof id);
        fprintf(out, "%s((%d))\n", id, i+1);
    }
    fputc('\n', out);

    for (int i = 0; i < G->n; ++i) {
        char A[32]; getId(i+1, A, sizeof A);
        for (Cell* c = G->adj[i].head; c; c = c->next) {
            char B[32]; getId(c->to, B, sizeof B);
            fprintf(out, "%s -->|%.2f|%s\n", A, c->p, B);
        }
    }
